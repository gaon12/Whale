...
원본 소스는 [GitHub.com](https://github.com/librewiki/whale-skin)에 존재하며, 버그 리포팅은 [bbs.librewiki.net](https://bbs.librewiki.net/)에서, 패치는 GitHub.com 에서만 받고 있습니다. 이 외의 호스팅에서 발견되는 코드는 모두 미러링이며, 코드의 최신성을 보증하지 않습니다.

보안 취약점은 이메일 (dev(골뱅이!)librewiki.net) 로 보고해 주세요.

## 설치

* 미디어위키 Skins 폴더에 압축을 풀거나 git clone을 수행하세요. 압축해제된 폴더의 이름은 `Whale` 이어야 합니다.
* LocalSettings.php 파일에 `wfLoadSkin( 'Whale' );` 를 추가해 주세요.
...
| `$wgWhaleOgLogo` | 오픈그래프 태그에 사용 될 이미지 설정 | `https://librewiki.net/images/6/6a/Libre_favicon.png` | `$wgLogo`의 값 |
| `$wgNaverVerification` | 네이버 사이트 도구 인증 코드 | (네이버에서 제공된 값) | (없음) |
| `$wgWhaleAdSetting` | 구글 애드센스 설정 | `array( 'client' => '(Google Adsense에서 제공한 값)', 'header' => '1234567890', 'right' => '0987654321', 'belowarticle' => 1313135452 )` | (없음) |
| `$wgWhaleAdGroup` | 사용자 그룹별 광고 차등화 여부 설정 | `differ` | `null`|
| `$wgWhaleMobileReplaceAd` | 모바일 환경일 시 사이드바 광고를 하단으로 옮깁니다. | `true` | `false` |
| `$wgWhaleEnableLiveRC` | 사이드바 최근 변경 사용 여부 | `true` | `true` |
| `$wgWhaleMaxRecent` | 사이드바 최근 변경에 등장하는 편집의 최대 개수 | `10` | `10` |
...
다음과 같은 형식을 따라서 `미디어위키:Whale-Navbar` 문서에 작성해주세요.
...
예시는 [리브레 위키](https://librewiki.net/wiki/MediaWiki:Whale-Navbar)에서 확인할 수 있습니다.

## 권한

권한별 광고 차등화를 구현하기 위해 아래와 같이 네 가지 권한이 추가됩니다. 만약 $wgWhaleAdGroup이 'differ'로 설정되어 있다면 아래 권한에 따라 환경설정에 광고 커스터마이징 옵션이 나타납니다.
